...
Pour plus d'informations : [Lien réseau ONDE (https://www.eaufrance.fr/observatoire-national-des-etiages)](https://www.eaufrance.fr/observatoire-national-des-etiages).
...
lib_mois <- month(lubridate::ym(annee_mois),label = TRUE,abbr = FALSE)
...
```{r map dpt nat, results='asis', echo=FALSE, crop=T, fig.width=5.5, fig.height=5, dpi=300}
...
# Bilan des écoulements pour les autres départements de la région

## Typologie nationale

```{r les autres dpts, results='asis', echo=FALSE, fig.width=11, fig.height=7, dpi=600}

## il y a plusieurs possibilités :
# - Soit dans le lancement, on fait expres de mettre un onde_df qui compte
# tous les departements et on obtient donc le tableau directement
# - soit on recupere les departements de la region et on telecharge les données de la region
# (avec quoi comme info ?)


produire_barplot_region_par_dpt(mois_sel = mois_sel,
                                annee_sel = params$annee_rapport,
                                region_dr = params$region_dr,
                                referentiel_onde = 'Typologie nationale',
                                complementaire = params$complementaire)

```

*Remarque : En fonction de la date d'édition du rapport, il est possible que tout les départements n'apparaissent pas.*
...
```{r graph indice mois, results='asis', echo=FALSE, fig.width=11, fig.height=7, dpi=600}

indice_onde_tab %>%
  dplyr::mutate(Mois = format(date_campagne, "%m")) %>%
  dplyr::filter(Mois == mois_sel) %>%
  arrange(Annee) %>%
  ungroup() %>%
  ggplot2::ggplot(.,
  ...
  ggplot2::scale_y_continuous(breaks = c(5:10)) +
  ...
  ggplot2::labs(x = paste0("Années de campagnes ONDE (mois de ", lib_mois,")"), y = "Valeurs d'indice",
                caption = paste0("Données Onde, au ", Sys.Date()),
                title = paste0("Notes d'indice pour le département (", lib_mois," 2012 - ",lubridate::year(Sys.Date()),")")) +
...
## Historique des types d'écoulements
...
params$onde_df %>%
  ...
  ggplot2::labs(title = "Durée des assecs : Proportions et nombre de stations concernées",
                    subtitle = "Campagnes usuelles") +
  ggplot2::theme(title = ggplot2::element_text(size = 9,face = 'bold'))
...
```{r historique stations, echo=FALSE, fig.height=3.3, fig.width=7, message=FALSE, dpi=300, results='asis'}
onde_dpt <- 
  params$onde_df %>% 
  dplyr::filter(code_departement == params$code_departement) %>% 
